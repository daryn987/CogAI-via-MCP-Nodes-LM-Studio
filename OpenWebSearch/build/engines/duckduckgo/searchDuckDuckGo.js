...
*/
async function searchDuckDuckGoPreloadUrl(query, maxResults = 10, proxyUrl) {
    const results = [];
    let offset = 0;
    try {
...
async function searchDuckDuckGoHtml(query, maxResults = 10, proxyUrl) {
    ...
    try {
        let response = await axios.post(requestUrl, new URLSearchParams({ q: query }).toString(), requestOptions);
        ...
        while (results.length < maxResults && items.length > 0) {
            offset += items.length;
            response = await axios.post(requestUrl, new URLSearchParams({
                q: query,
                s: offset.toString(),
                dc: offset.toString(),
                v: 'l',
