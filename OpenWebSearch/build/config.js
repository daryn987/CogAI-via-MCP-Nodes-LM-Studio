...
export const config = {
    ...
    defaultSearchEngine: process.env.DEFAULT_SEARCH_ENGINE || 'bing',
...
if (config.allowedSearchEngines.length > 0) {
    ...
    // Check if default engine is in the allowed list
        ...
        config.defaultSearchEngine = config.allowedSearchEngines[0];
...
export function getProxyUrl() {
    return config.useProxy ? encodeURI(config.proxyUrl) : undefined;
}
